...
def test_blocks():
    ...
    # 45 bloques en total (9 definiciones × 5 días)
    check(N_BLOQUES == 45, f"45 bloques totales (hay {N_BLOQUES})")

    # Bloques de 3h solo en 10:30 y 12:30
    ...
    # Solapamiento conocido: 14:30-16:20 y 15:30-17:20 mismo día → solapan
    lunes_b = [b for b in TODOS_BLOQUES if b.dia.value == "L"]
    b1 = next(b for b in lunes_b if b.hora_inicio == "14:30" and b.tipo == "2h")
    b2 = next(b for b in lunes_b if b.hora_inicio == "15:30" and b.tipo == "2h")
    check(bloques_se_solapan(b1, b2), "14:30-16:20 y 15:30-17:20 (L) se solapan")

    # Solapamiento conocido: 10:30-13:20 y 12:30-15:20 → solapan (comparten 12:30)
    b3 = next(b for b in lunes_b if b.hora_inicio == "10:30" and b.tipo == "3h")
    b4 = next(b for b in lunes_b if b.hora_inicio == "12:30" and b.tipo == "3h")
    check(bloques_se_solapan(b3, b4), "10:30-13:20 y 12:30-15:20 (L) se solapan")

    # No solapan: 8:30-10:20 y 10:30-12:20
    b5 = next(b for b in lunes_b if b.hora_inicio == "8:30")
    b6 = next(b for b in lunes_b if b.hora_inicio == "10:30" and b.tipo == "2h")
    check(not bloques_se_solapan(b5, b6), "8:30-10:20 y 10:30-12:20 NO se solapan")

    # No solapan: mismo slot, días distintos
    lunes_830  = next(b for b in TODOS_BLOQUES if b.dia.value == "L" and b.hora_inicio == "8:30")
    martes_830 = next(b for b in TODOS_BLOQUES if b.dia.value == "M" and b.hora_inicio == "8:30")
    check(not bloques_se_solapan(lunes_830, martes_830),
          "8:30 L y 8:30 M NO se solapan (días distintos)")
...
def test_cursos(datos):
    ...
    # 223 cursos únicos en la unión de los 3 catálogos
    n = len(datos.cursos)
    check(n == 223, f"223 cursos únicos (hay {n})")

    # Cada plan tiene el número correcto de cursos
    conteo = {}
    for curso in datos.cursos.values():
        for plan in curso.semestres_por_plan:
            conteo[plan] = conteo.get(plan, 0) + 1

    check(conteo.get("PE2022", 0) == 141, f"PE2022 tiene 141 cursos (hay {conteo.get('PE2022', 0)})")
    check(conteo.get("PE2022_2025", 0) == 144, f"PE2022_2025 tiene 144 cursos (hay {conteo.get('PE2022_2025', 0)})")
    check(conteo.get("PE2026", 0) == 168, f"PE2026 tiene 168 cursos (hay {conteo.get('PE2026', 0)})")

    # ING1100 es Plan Común semestre 1 en PE2022 y PE2022_2025
    # (no está en PE2026 — fue reemplazado en ese plan)
    ings = datos.cursos.get("ING1100")
    check(ings is not None, "ING1100 existe")
    for plan in ["PE2022", "PE2022_2025"]:
        sem = ings.semestres_por_plan.get(plan, {})
        check("Plan Común" in sem, f"ING1100 tiene Plan Común en {plan}")
        check(sem.get("Plan Común") == 1, f"ING1100 semestre 1 en {plan} (es {sem.get('Plan Común')})")

    # ING1100 debe tener clases_horas > 0
    check(ings.clases_horas > 0, f"ING1100 clases_horas={ings.clases_horas} > 0")

    # Cursos Plan Común NO deben tener carreras de especialidad en semestres_por_plan
    for codigo, curso in datos.cursos.items():
        for plan, semestres in curso.semestres_por_plan.items():
            if "Plan Común" in semestres:
                carreras_esp = [c for c in semestres if c != "Plan Común"]
                check(len(carreras_esp) == 0,
                      f"{codigo} ({plan}): cursos Plan Común no deben tener carreras especialidad {carreras_esp}")
...
def test_secciones(datos):
    ...
    n = len(secciones)
    check(n == 304, f"304 secciones totales (hay {n})")
    ...
    # Todas las AYUD tienen afecta_disponibilidad = False
    ...
    # Todas las CLAS/LABT tienen afecta_disponibilidad = True
...
def test_horas_a_bloques(datos):
    ...
    for s in ing1100_clas:
        check(s.cantidad_bloques_necesarios == 4,
              f"ING1100 CLAS tiene 4 bloques (seccion {s.seccion}: {s.cantidad_bloques_necesarios})")

    # Secciones con 3h de clases → 1 bloque de 3h
    ...
    # Secciones con 2h de clases → 1 bloque de 2h
    cursos_2h = [c for c in cursos.values() if c.clases_horas == 2]
    for curso in cursos_2h:
        for s in secciones:
            if s.codigo_curso == curso.codigo and s.componente == TipoReunion.CLAS:
                check(s.cantidad_bloques_necesarios == 1,
                      f"{curso.codigo} (2h) CLAS → 1 bloque (es {s.cantidad_bloques_necesarios})")
...
def test_profesores(datos):
    ...
    n = len(profesores)
    check(n == 130, f"130 profesores (hay {n})")
    ...
    # Todo rut de sección debe existir en profesores
...
def test_salas_especiales(datos):
    ...
    check(len(cursos_con_sala) > 0, "Hay cursos con sala especial asignada")

    # ING1100 no tiene sala especial (es clase normal)
    ...
    # ICC4101 sí tiene sala especial
