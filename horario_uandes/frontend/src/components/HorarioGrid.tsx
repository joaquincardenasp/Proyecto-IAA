...
function getSemestresForCarrera(
  sec: SeccionAsignada,
  carrera: string,
): string[] {
...
function aplicarFiltros(
  ...
  return secciones.filter((sec) => {
    ...
    if (filters.carrera && !sec.carreras.includes(filters.carrera))
      return false;
    ...
    if (
      q &&
      ![sec.codigo, sec.titulo, sec.profesor, sec.seccion]
        .join(" ")
        .toLowerCase()
        .includes(q)
    )
...
function buildPlacements(
  filtered: SeccionAsignada[],
): Record<Dia, Placement[]> {
...
interface Props {
  secciones: SeccionAsignada[];
  bloquesCatalogo?: BloqueCatalogo[];
  editable?: boolean;
  seccionesConViolacion?: Set<string>;
  onMoverBloque?: (
    secId: string,
    bloqueIdx: number,
    nuevoDia: Dia,
...
export default function HorarioGrid({
  ...
  const availableSems = useMemo(() => {
    ...
    for (const sec of secciones)
      for (const s of getSemestresForCarrera(sec, filters.carrera)) set.add(s);
    return Array.from(set).sort(compareSems);
  }, [secciones, filters.carrera]);

  const filtered = useMemo(
    () => aplicarFiltros(secciones, filters),
    [secciones, filters],
  );
  ...
  function snapToBlock(
    dia: Dia,
    horaInicioSoltada: string,
    duracionMin: number,
  ): BloqueCatalogo | null {
    const minSoltado = toMin(horaInicioSoltada);
    const candidatos = bloquesCatalogo.filter((b) => {
      if (b.dia !== dia) return false;
      const dur = toMin(b.hora_fin) - toMin(b.hora_inicio);
      return dur === duracionMin;
    });
    if (candidatos.length === 0) return null;
    return candidatos.reduce((best, c) =>
      Math.abs(toMin(c.hora_inicio) - minSoltado) <
      Math.abs(toMin(best.hora_inicio) - minSoltado)
        ? c
        : best,
    );
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-0.5 border-t border-gray-100">
          <div className="flex gap-1">
            {(["", "CLAS", "AYUD", "LABT"] as const).map((t) => (
              <button
                key={t}
                onClick={() =>
                  setFilters((f) => ({ ...f, tipo: t as TipoSeccion | "" }))
                }
          ...
          <div
            className="flex items-center gap-2 flex-1 min-w-48 border border-gray-200
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white">
        <div className="flex" style={{ minWidth: 900 }}>
          ...
          {DIAS.map((d) => (
            <div
              key={d.key}
              className="flex-1 min-w-0 border-l border-gray-200 first:border-l-0"
            >
              <div
              ...
              <div
                ...
                onDrop={
                  editable
                    ? (e) => {
                        e.preventDefault();
                        if (!dragging || !onMoverBloque) return;
                        const rect = e.currentTarget.getBoundingClientRect();
                        const y = e.clientY - rect.top;
                        let row = Math.floor(y / ROW_H);
                        row = Math.max(
                          0,
                          Math.min(SUB_BLOQUES.length - 1, row),
                        );
                        onMoverBloque(
                          dragging.secId,
                          dragging.bloqueIdx,
                          d.key,
                          SUB_BLOQUES[row].inicio,
                        );
                        setDragging(null);
                      }
                    : undefined
              ...
              >
                {/* Filas visuales (sin lógica de drop, solo fondo alternado) */}
                {SUB_BLOQUES.map((sb, i) => (
                  <div
                    key={i}
            ...
            ${i % 2 === 1 ? "bg-gray-50/50" : ""}
            ${editable && dragging ? "bg-blue-50/30" : ""}`}
                  />
                ))}
                  ...
                  >
                    {p.sections.length === 1 ? (
                      <SeccionCard
                        ...
                        onClick={() =>
                          setSelected(
                            p.sections[0] === selected ? null : p.sections[0],
                          )
                        }
                        editable={editable}
                        violacion={seccionesConViolacion?.has(p.sections[0].id)}
                        onDragStart={() => {
                          const bIdx = p.sections[0].bloques.findIndex(
                            (b) =>
                              b.dia === d.key &&
                              b.hora_inicio ===
                                SUB_BLOQUES[Math.round(p.startRow)].inicio,
                          );
                          setDragging({
                            secId: p.sections[0].id,
                            bloqueIdx: bIdx >= 0 ? bIdx : 0,
                          });
                        }}
                      />
                    ) : (
                      <GroupBlock
                        ...
                        dia={d.key}
                        startRow={p.startRow}
                        onDragStartSec={(secId) => {
                          const sec = p.sections.find((s) => s.id === secId)!;
                          const bIdx = sec.bloques.findIndex(
                            (b) =>
                              b.dia === d.key &&
                              b.hora_inicio ===
                                SUB_BLOQUES[Math.round(p.startRow)].inicio,
      ...
      <div className="flex flex-wrap gap-5 text-xs text-gray-500">
        {(["CLAS", "AYUD", "LABT"] as TipoSeccion[]).map((t) => (
          <span key={t} className="flex items-center gap-1.5">
            <span
              className={`inline-block w-2.5 h-2.5 rounded-sm ${TIPO_TAG[t]}`}
            />
            {TIPO_LABEL[t]}
          </span>
        ))}
...
function GroupBlock({
  ...
  dia?: Dia;
  startRow?: number;
  onDragStartSec?: (secId: string) => void;
}) {
  const tipo = group[0].tipo;

  return (
    <div
      className={`h-full rounded overflow-y-auto flex flex-col ${TIPO_CARD[tipo]}`}
    >
      {group.map((sec, idx) => (
        <button
          key={sec.id}
          id={`sec-${sec.id}`}
          draggable={editable}
          onDragStart={editable ? () => onDragStartSec?.(sec.id) : undefined}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {sec.bloques.map((b, i) => (
            ...
            >
              {DIAS_LABEL[b.dia as Dia]} · {b.hora_inicio}–{b.hora_fin} (
              {b.tipo_bloque})
            </span>
          ))}
        </div>
